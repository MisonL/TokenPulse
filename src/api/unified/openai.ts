...
openaiCompat.post('/chat/completions', async (c) => {
    ...
    if (['gemini', 'antigravity', 'aistudio'].includes(provider)) {
        const { contents, systemInstruction } = Translators.openAIToGemini(body.messages);
        upstreamPayload = {
            model: targetModel,
    ...
    try {
        ...
        if (['gemini', 'antigravity', 'aistudio'].includes(provider)) {
            if (body.stream) {
                const { GoogleToOpenAITranslator } = await import('../../lib/translator/google_to_openai');
                if (resp.body) {
                    const googleStream = resp.body as ReadableStream<Uint8Array>;
                    const iterator = GoogleToOpenAITranslator.translateStream(googleStream, targetModel);
                ...
                const { GoogleToOpenAITranslator } = await import('../../lib/translator/google_to_openai');
                const openaiJson = GoogleToOpenAITranslator.translateResponse(googleJson, targetModel);
        ...
        return c.json({ error: `Gateway dispatch failed to ${provider}`, details: String(e) }, 502);
    }
});
