...
export async function initiateKiroDeviceFlow(clientId: string, clientSecret: string): Promise<DeviceAuthResponse> {
    const payload = {
        clientId,
        clientSecret,
        startUrl: START_URL
...
export async function pollKiroToken(
    ...
    clientId: string, 
    clientSecret: string
): Promise<{ success: boolean; accessToken?: string; pending?: boolean; error?: string }> {
    const payload = {
        clientId,
        clientSecret,
        deviceCode,
