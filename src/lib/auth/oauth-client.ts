...
export class OAuthService {
    ...
    public async exchangeCodeForToken(code: string, verifier?: string, authMethod: 'body' | 'basic' = 'body'): Promise<TokenResponse> {
        ...
        const headers: Record<string, string> = {
            'Content-Type': authMethod === 'body' ? 'application/json' : 'application/x-www-form-urlencoded',
            'Accept': 'application/json'
        };

        if (authMethod === 'basic' && this.config.clientSecret) {
            const authPrefix = Buffer.from(`${this.config.clientId}:${this.config.clientSecret}`).toString('base64');
        ...
        return this.fetchWithRetry(this.config.tokenUrl, {
            method: 'POST',
            headers,
            body: authMethod === 'body' ? JSON.stringify(body) : new URLSearchParams(body).toString()
        });
    }
     ...
     */
    public async initiateDeviceFlow(deviceAuthUrl: string, extraParams: any = {}): Promise<DeviceAuthResponse> {
        const body = {
            client_id: this.config.clientId,
            scope: this.config.scopes.join(' '),
            ...extraParams
     ...
     * 能力：轮询设备令牌
     */
    public async pollDeviceToken(deviceCode: string, extraParams: any = {}): Promise<TokenResponse> {
        const body = {
            client_id: this.config.clientId,
    ...
    private async fetchWithRetry(url: string, options: RequestInit, retries = 5, initialDelay = 1000): Promise<TokenResponse> {
        let lastError: any;
            ...
            try {
                ...
                if (!res.ok) {
                    ...
                    if (res.status >= 400 && res.status < 500) {
                        throw new Error(`Auth client error (${res.status}): ${text} -- NO_RETRY`);
                    }
                    throw new Error(`Auth request failed (${res.status}): ${text}`);
                }
                ...
                if (i < retries - 1) {
                    ...
                    logger.warn(`Auth request failed (attempt ${i+1}/${retries}). Retrying in ${Math.round(delay)}ms... Error: ${e.message}`);
    ...
    public async saveCredentials(data: TokenResponse, email?: string, metadata: any = {}) {
        const now = Date.now();
        ...
        await db.insert(credentials).values({
            ...
            nextRefreshAfter: expiresAt ? (now + (data.expires_in * 1000 * 0.8)) : undefined
        }).onConflictDoUpdate({
            ...
            set: {
                ...
                status: 'active',
                nextRefreshAfter: expiresAt ? (now + (data.expires_in * 1000 * 0.8)) : undefined, // Refresh at 80% lifetime
                attributes: JSON.stringify(metadata.attributes || {})
            }
        });
    }
