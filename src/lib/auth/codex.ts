...
export function startCodexCallbackServer() {
    Bun.serve({
        ...
        async fetch(req) {
            ...
            if (!code || !state) {
                return new Response("<h1>Missing Code or State</h1>", { headers: { 'Content-Type': 'text/html' } });
            }

            const verifier = pendingStates.get(state);
            if (!verifier) {
                return new Response("<h1>Invalid or Expired State</h1>", { headers: { 'Content-Type': 'text/html' } });
            }
