...
const BASE_BETAS = "claude-code-20250219,oauth-2025-04-20,interleaved-thinking-2025-05-14,fine-grained-tool-streaming-2025-05-14";
...
class ClaudeProvider extends BaseProvider {
    ...
    protected override async transformRequest(body: ChatRequest, headers: any, context?: any): Promise<any> {
        ...
        if (tools) {
            ...
            if (body.tool_choice) {
                ...
                } else if (body.tool_choice === 'required') { // 'required' -> 'any' in Claude
                     payload.tool_choice = { type: 'any' };
                } else if (typeof body.tool_choice === 'object' && (body.tool_choice as any).type === 'function') {
                      payload.tool_choice = { type: 'tool', name: (body.tool_choice as any).function.name };
                }
            }
        }
        ...
        const userHash = crypto.createHash('sha256').update(account + session).digest('hex').substring(0, 16);
        payload.metadata = {
    ...
    protected override async fetchUserInfo(token: string): Promise<{ email?: string; id?: string; attributes?: any }> {
        // 1. Get User/Org Info
        let email = undefined;
        let id = undefined;
