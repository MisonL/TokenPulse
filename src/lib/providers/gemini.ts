...
gemini.get('/auth/url', (c) => {
  ...
  // Set Cookie for CSRF
  c.header('Set-Cookie', `gemini_oauth_state=${state}; HttpOnly; Path=/; Max-Age=300`);
...
gemini.get('/oauth2callback', async (c) => {
  ...
  const cookieState = c.req.header('Cookie')?.match(/gemini_oauth_state=([^;]+)/)?.[1];
  ...
  if (!tokenResp.ok) return c.json({ error: 'Token exchange failed', details: await tokenResp.text() }, 400);
...
gemini.post('/v1/chat/completions', async (c) => {
  const creds = await db.select().from(credentials).where(eq(credentials.provider, PROVIDER_ID)).limit(1);
  const cred = creds[0];
