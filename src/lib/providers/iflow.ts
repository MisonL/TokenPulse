...
export class IFlowProvider extends BaseProvider {
    ...
    protected override async handleCallback(c: any) {
        ...
        try {
            // 使用 'basic' 模式进行令牌交换
            const tokenData = await this.oauthService.exchangeCodeForToken(code, undefined, 'basic');
    ...
    protected override async fetchUserInfo(token: string): Promise<any> {
        try {
            const resp = await fetch(`https://iflow.cn/api/oauth/getUserInfo?accessToken=${token}`);
            if (resp.ok) {
