...
import { AgenticSystemPrompt, checkThinkingMode, shortenToolName } from './utils';
...
export class KiroProvider extends BaseProvider {
    ...
    private async ensureClientRegistered(): Promise<void> {
        // 1. 尝试从注册中心读取缓存
        const cached = await InfrastructureRegistry.get<{ clientId: string, clientSecret: string }>('kiro_client_creds');
        if (cached) {
        ...
        try {
            ...
            if (!regResp.ok) throw new Error("AWS OIDC Register Failed: " + await regResp.text());
            
            const data = await regResp.json() as any;
            this.authConfig.clientId = data.clientId;
            this.authConfig.clientSecret = data.clientSecret;

            await InfrastructureRegistry.set('kiro_client_creds', {
    ...
    protected override async fetchUserInfo(token: string): Promise<any> {
        ...
        try {
            ...
            logger.warn(`Kiro: Failed to fetch ProfileInfo: ${e instanceof Error ? e.message : e}`);
        }
        ...
        if (!identity.profileArn) {
            try {
                const resp = await fetch('https://codewhisperer.us-east-1.amazonaws.com', {
                    ...
                    headers: {
                        'Content-Type': 'application/x-amz-json-1.1',
                        'X-Amz-Target': 'AmazonCodeWhispererService.ListAvailableCustomizations',
                        'Authorization': `Bearer ${token}`
                    },
                    body: JSON.stringify({ })
                });
        ...
        // 3. 尝试获取通用用户信息
        try {
            const resp = await fetch('https://profile.aws.amazon.com/v1/user/details', {
            ...
            logger.warn(`Kiro: Failed to fetch generic user details: ${e instanceof Error ? e.message : e}`);
        }
    ...
    private static readonly SOCIAL_AUTH_ENDPOINT = "https://prod.us-east-1.auth.desktop.kiro.dev";
    ...
    protected override async handleAuthUrl(c: Context) {
        ...
        if (method === 'google' || method === 'github') {
            return this.handleSocialAuthUrl(c, method === 'google' ? 'Google' : 'Github');
        }

        try {
            await this.ensureClientRegistered();
            const deviceResp = await this.oauthService.initiateDeviceFlow(OIDC_AUTH_URL, {
    ...
    protected override async handleCallback(c: any) {
        ...
        if (code) {
             ...
             try {
                const tokenResp = await fetch(`${KiroProvider.SOCIAL_AUTH_ENDPOINT}/oauth/token`, {
                    method: 'POST',
    ...
    protected override async getCustomHeaders(token: string, body: any, context?: any): Promise<Record<string, string>> {
        return {
            ...
            'X-Amz-Target': body.target || 'AmazonQDeveloperStreamingService.SendMessage',
    ...
    protected override async transformRequest(body: ChatRequest, headers?: any, context?: any): Promise<any> {
        ...
        systemPrompt = systemPrompt ? timestampContext + "\n\n" + systemPrompt : timestampContext;

        if (body.response_format) {
             let responseFormatHint = "";
             if (body.response_format.type === 'json_object') {
                 responseFormatHint = "[INSTRUCTION: Respond ONLY with a valid JSON object. Do not include any explanations or text outside the JSON.]";
        ...
        if (body.tool_choice) {
            ...
            if (body.tool_choice === 'none') {
                toolChoiceHint = "[INSTRUCTION: Do NOT use any tools. Respond with text only.]";
            } else if (body.tool_choice === 'required') {
                toolChoiceHint = "[INSTRUCTION: You MUST use at least one of the available tools.]";
        ...
        for (let i = 0; i < messages.length; i++) {
            ...
            if (msg.role === 'user') {
                const userMsg = {
                    userInputMessage: {
                        ...
                        origin: "CLI",
                        userInputMessageContext: pendingToolResults.length > 0 ? { toolResults: [...pendingToolResults] } : undefined
                    }
                };
                pendingToolResults = [];
                if (isLast) currentMessage = userMsg; else history.push(userMsg);
            }
            ...
            if (msg.role === 'assistant') {
                ...
                if (isLast) {
                    currentMessage = {
                         userInputMessage: { content: "Continue", modelId: body.model || "amazon-q", origin: "CLI" }
                    };
                }
            }
        }
        ...
        if (pendingToolResults.length > 0 && currentMessage) {
            if (!currentMessage.userInputMessage.userInputMessageContext) currentMessage.userInputMessage.userInputMessageContext = {};
            currentMessage.userInputMessage.userInputMessageContext.toolResults = pendingToolResults;
        }
        ...
        if (currentMessage) {
            ...
            if (kiroTools.length > 0) {
                 if (!currentMessage.userInputMessage.userInputMessageContext) currentMessage.userInputMessage.userInputMessageContext = {};
                 currentMessage.userInputMessage.userInputMessageContext.tools = kiroTools;
            }
        }
