...
function getEncryptionKey(): Buffer {
  ...
  if (!secret) {
    throw new Error('ENCRYPTION_SECRET environment variable is required for encryption');
  }

  // 使用 PBKDF2 从环境变量派生密钥
  const salt = crypto.createHash('sha256').update('tokenpulse-salt').digest();
  return crypto.pbkdf2Sync(secret, salt, ENCRYPTION_CONFIG.iterations, ENCRYPTION_CONFIG.keyLength, 'sha256');
}
