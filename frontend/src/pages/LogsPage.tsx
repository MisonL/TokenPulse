    ...
    <div className="space-y-6">
       <div className="flex items-center justify-between border-b-4 border-black pb-4">
          <div className="flex items-center gap-4">
            ...
            <h2 className="text-4xl font-black uppercase text-black">{t('logs.title')}</h2>
          </div>
          <div className="flex gap-4">
           <div className="relative">
               <label htmlFor="log-search" className="sr-only">{t('common.search_logs')}</label>
               <input
                   id="log-search"
                   name="log-search"
                   type="text"
              ...
              <button onClick={() => setRefreshKey(k => k + 1)} className="b-btn b-btn-primary p-2">
       ...
       <div className="bg-white border-4 border-black p-0 overflow-hidden b-shadow">
          <table className="w-full text-left font-mono text-sm">
             <thead className="bg-black text-white">
                <tr>
                   <th className="p-4 uppercase tracking-wider border-r border-white/20">{t('logs.table_time')}</th>
                   <th className="p-4 uppercase tracking-wider border-r border-white/20">{t('logs.table_level')}</th>
                   <th className="p-4 uppercase tracking-wider border-r border-white/20">{t('logs.table_source')}</th>
                   <th className="p-4 uppercase tracking-wider">{t('logs.table_msg')}</th>
                </tr>
             </thead>
             <tbody className="divide-y-2 divide-black">
                {filteredLogs.length === 0 && (
                    <tr><td colSpan={4} className="p-8 text-center text-gray-400 italic">{t('dashboard.no_events')}</td></tr>
                )}
                {filteredLogs.map((log) => (
                   <tr key={log.id} className="hover:bg-yellow-50 transition-colors group">
                      ...
                      <td className="p-4 border-r-2 border-black font-bold">
                         <span className={
                           log.level === 'INFO' ? 'text-[#005C9A]' :
                           log.level === 'WARN' ? 'text-[#e5b300]' :
                           'text-[#DA0414]'
                         }>{log.level}</span>
                      </td>
                      <td className="p-4 border-r-2 border-black font-semibold">{log.source}</td>
          ...
          <div className="p-4 border-t-4 border-black bg-gray-50 flex justify-between items-center">
              ...
              <span className="font-mono text-xs">{t('common.page_info', { current: page.toString(), total: totalPages.toString() })}</span>
              <button 
