  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex items-end justify-between border-b-4 border-black pb-4">
        <h2 className="text-4xl font-black uppercase text-black">{t('dashboard.title')}</h2>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 h-96">
         ...
         <div className="lg:col-span-2 bg-white border-4 border-black p-6 relative overflow-hidden group">
            <h3 className="text-2xl font-bold uppercase mb-4 z-10 relative">{t('dashboard.traffic_chart')}</h3>
               ...
               {stats.traffic_history.map((h, i) => (
                 <div 
                   ...
                   className={cn(
                     ...
                     i % 3 === 0 ? "bg-[#DA0414]" : i % 3 === 1 ? "bg-[#005C9A]" : "bg-[#FFD500]"
                   )}
                   style={{ height: `${h > 0 ? (h / Math.max(...stats.traffic_history, 10)) * 100 : 5}%` }} 
                 />
               ))}
            </div>
         </div>
...
function RecentLogsWidget() {
    ...
    return (
        <div className="bg-[#1A1A1A] border-4 border-black p-6 text-white overflow-hidden flex flex-col">
            <h3 className="text-xl font-bold uppercase mb-6 text-[#FFD500] border-b border-white/20 pb-2">{t('dashboard.recent_events')}</h3>
            <div className="space-y-4 font-mono text-xs flex-1">
               {logs.length === 0 && <div className="text-gray-500 italic">{t('dashboard.no_events')}</div>}
               {logs.map(log => (
                   <LogItem 
                    key={log.id} 
...
function StatWidget({ label, value, icon, color, textColor }: StatWidgetProps) {
  return (
    <div className={cn("border-4 border-black p-6 flex flex-col justify-between h-40 transition-transform hover:-translate-y-1 b-shadow", color, textColor)}>
       <div className="flex justify-between items-start">
         <span className="font-mono text-xs font-bold uppercase tracking-widest opacity-70">{label}</span>
         {icon}
       </div>
  ...
  return (
    <div className="flex gap-3 border-l-2 border-white/10 pl-3 py-1">
      ...
      <span className={cn(
        ...
        type === 'warning' ? "from-[#FFD500] to-orange-400" :
