...
describe("企业域管理员认证与 RBAC 回归", () => {
  ...
  it("login 成功应 set-cookie 且写入审计事件", async () => {
    ...
    const sessions = (sessionRows as unknown as { rows?: Array<{
      id: string;
      user_id: string;
      role_key: string;
    ...
    const audits = (auditRows as unknown as { rows?: Array<{
      actor: string;
      action: string;
      resource: string;
  ...
  it("普通角色可删（删除后 admin_roles/admin_user_roles 均应清理）", async () => {
    ...
    const roles = (roleRows as unknown as { rows?: Array<{ key: string }> }).rows || [];
