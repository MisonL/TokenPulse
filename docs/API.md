...
`POST/PUT /api/admin/billing/policies*` 的 `scopeType/scopeValue` 规则：`global` 不允许传 `scopeValue`；`tenant`/`project`/`role`/`user` 必须传非空 `scopeValue`。
其中 `tenant` 会将 `scopeValue` 归一化为小写并校验租户是否存在；`project` 会将 `scopeValue` 归一化为小写并校验项目是否存在；`role` 会先将 `scopeValue` 转为小写再校验角色是否存在；`user` 会将 `scopeValue` 转为小写并校验管理员用户是否存在。
校验失败返回 `400`（如 `scopeType=global` 仍传 `scopeValue`、或 `tenant/project/role/user` 缺少 `scopeValue`）；租户/项目/角色/用户不存在返回 `404`；错误体均包含 `{ error, traceId }`。
...
> `GET /api/admin/observability/oauth-alerts/incidents` 支持筛选参数：`provider/phase/severity/from/to/page/pageSize`；响应中的 `incidentId` 为稳定独立字符串，并统一使用 `incident:` 命名空间，同一告警事故的 warning/critical/recovery 事件会复用该值。
> `GET /api/admin/observability/oauth-alerts/deliveries` 支持筛选参数：`eventId/incidentId/provider/phase/severity/channel/status/from/to/page/pageSize`，`eventId` 与 `incidentId` 可独立传入或同时取交集；`status` 查询兼容 `success|failure|sent|failed`，响应统一为 `success|failure`。历史 `legacy:*` / `provider:phase:eventId` 记录会在响应中归一为 `incident:*`，并可继续用归一后的 `incidentId` 查询命中。
...
- `X-TokenPulse-Project`（或 `X-TokenPulse-Project-Id` / `X-Project-Id`）：仅在 `TRUST_PROXY=true` 时用于项目上下文透传（配额与 AgentLedger 运行时摘要联查）；未启用可信代理时会被忽略。
