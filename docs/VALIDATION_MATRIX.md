...
| Enterprise + AgentLedger 统一最小验收入口 | `./scripts/release/validate_enterprise_runtime_bundle.sh --env-file ... --evidence-file ./artifacts/enterprise-runtime-bundle-evidence.json` | 适用于联调前收口、灰度前最小验收、值班交接前复核；默认固定顺序执行 `check_enterprise_boundary.sh`、`drill_agentledger_runtime_webhook.sh`，并且只有显式传入 `--with-post-canary=true` 时才追加 `canary_gate.sh --phase post --with-boundary true --with-smoke false`；evidence 至少包含 `overallStatus/baseUrl/envFile/withPostCanary/startedAt/finishedAt/steps[].name/status/command/startedAt/finishedAt/exitCode/evidenceFile`；不替代 `release_window_oauth_alerts.sh` 的真实发布窗口证据 |
